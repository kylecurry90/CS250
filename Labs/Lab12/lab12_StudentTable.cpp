...
int StudentTable::HashFunction( int key )
{
	...
	while (m_students[index].key != -1)
	{
		...
		{
			index = QuadraticProbe(origianlIndex, quadraticAdd);
			index = index % TABLE_SIZE;
		}
		if (m_collisionMethod == DOUBLE)
		{
			doubleHashAdd = doubleHashAdd + HashFunction2(origianlIndex);
			index = (origianlIndex + doubleHashAdd) % TABLE_SIZE;
		}
	}
	return index;
}
