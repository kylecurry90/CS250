...
public:
    ...
    */
    ...
    /**
		...
		Code used from "Data Structures and Algorithms for Game Developers" by Allen Sherrod.
    */
	void Delete(const TK& key)
	{
		...
		if (parent->ptrLeft->key == node->key)
		{
			isLeftNode = true;
		}
...
private:
    ...
    */
    Node<TK, TD>* FindParentOfNode( const TK& key )
    {
		...
		Node<TK, TD>* parentNode = m_ptrRoot;
		while (parentNode != nullptr)
		{
			if ((parentNode->ptrLeft != nullptr && parentNode->ptrLeft->key == key) || (parentNode->ptrRight != nullptr && parentNode->ptrRight->key == key))
			{
				return parentNode;
			}
			else
			{
				...
				{
					parentNode = parentNode->ptrLeft;
				}
				else if (key > parentNode->key)
				{
					parentNode = parentNode->ptrRight;
				}
			}
		}
		return nullptr;
    ...
    */
    int GetHeight( Node<TK, TD>* ptrCurrent )
    {
		...
		{
			int leftSubTree = GetHeight(ptrCurrent->ptrLeft);
			int rightSubTree = GetHeight(ptrCurrent->ptrRight);
